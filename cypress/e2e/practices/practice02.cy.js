...
describe('Practice01 Validate Google Home Page', () => {
  ...
  it('TASK-3: Validate the Amazon Search Results', () => {
    ...
    cy.get('.a-section.a-spacing-small.a-spacing-top-small span')
      .eq(0)
      .then(($el) => {
        const str = $el.text()
        expect(
  ...
  it.only('TASK-4: Validate the Google Amazon Footer Columns', () => {
    // Step 2
    const footer = ['Get to Know Us', 'Make Money with Us', 'Amazon Payment Products', 'Let Us Help You']
    cy.get('.navFooterColHead').each(($el, index) => {
      cy.wrap($el).should('have.text', footer[index])
    })
  })
})
