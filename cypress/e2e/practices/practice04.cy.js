...
describe('Static Tables', () => {
  ...
  it('TASK-2: Validate the GitHub Home Page Search and Signing Header Items', () => {
    gitHubHomePage.getSearchInput().should('have.attr', 'placeholder', 'Search or jump to...' )
  ...
  it('TASK-3: Validate the GitHub Login Page Sign in Form', () => {
    ...
    gitHubLoginPage.getPasskeyButton().should('have.text', 'Sign in with a passkey')
    gitHubLoginPage.getCreateAnAccountLink().should('have.text', 'Create an account').and('be.visible')
    gitHubLoginPage.getSignInButton().should('be.visible').and('contain', 'Sign in')
  })

  it.only('TASK-4: Validate the GitHub Login Page Footer Links ', () => {
    gitHubHomePage.clickSignInButton()
    gitHubHomePage.clickSignInButton()

    const footerLinks = ['Terms', 'Privacy', 'Docs', 'Contact GitHub Support', 'Manage cookies', 'Do not share my personal information']
    gitHubLoginPage.getFooterInfo().each(($el, index) => {
